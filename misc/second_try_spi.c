...
void IRAM_ATTR spi_post_trans_cb(spi_slave_transaction_t *t) {
    
    counter++;

    // Immediately re-queue this transaction
...
void app_main(void)
{
    ...
    // SPI for RX
    gpio_set_pull_mode(DATA_IN_PIN, GPIO_PULLUP_ONLY);
    gpio_set_pull_mode(CLK_PIN, GPIO_PULLUP_ONLY);
    gpio_set_pull_mode(23, GPIO_PULLUP_ONLY);

    spi_bus_config_t buscfg_rx = {
        .mosi_io_num = DATA_IN_PIN,
        .miso_io_num = -1,
        .sclk_io_num = CLK_PIN,
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
    };
    spi_slave_interface_config_t slvcfg_rx = {
        .mode = 0,                    // SPI mode 0 (adjust as needed)
        .spics_io_num = 23,
        .queue_size = 2,              // Can queue 2 transactions
        .flags = 0,
        .post_setup_cb = NULL,
        .post_trans_cb = spi_post_trans_cb,  // Your callback
    };
    ESP_ERROR_CHECK(spi_slave_initialize(SPI2_HOST, &buscfg_rx, &slvcfg_rx, SPI_DMA_CH_AUTO));

    // Prepare initial transactions
    for(int i = 0; i < 2; i++) {
        memset(&trans[i], 0, sizeof(spi_slave_transaction_t));
        trans[i].length = 32;        // 32 bits = 4 bytes
        trans[i].rx_buffer = rx_buf[i];
        trans[i].tx_buffer = NULL;
        
        // Queue the transaction
        ESP_ERROR_CHECK(spi_slave_queue_trans(SPI2_HOST, &trans[i], portMAX_DELAY));
    }
